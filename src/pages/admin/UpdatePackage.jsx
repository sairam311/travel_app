...
const uploadImageToCloudinary = async (file) => {
  const cloudName = import.meta.env.CLOUDINARY; // Replace with your Cloudinary cloud name
  const uploadPreset = "ai-travel"; // Replace with your Cloudinary upload preset
  ...
  try {
    const response = await fetch(`${cloudName}`, {
      method: "POST",
      body: formData,
    });
    const data = await response.json();
    return data.secure_url; // This will be the Cloudinary URL for the uploaded image
  } catch (error) {
...
const UpdatePackage = () => {
  ...
  const getPackageData = async () => {
    try {
      const res = await fetch(`${import.meta.env.VITE_SERVER_URL}/api/package/get-package-data/${params?.id}`);
  ...
  const handleImageSubmit = () => {
    if (images.length > 0 && images.length + formData.packageImages.length < 6) {
      ...
      const oversizedImages = Array.from(images).filter((file) => file.size > maxSize);
  ...
  const storeImage = async (file) => {
    return await uploadImageToCloudinary(file); // Uploading image to Cloudinary and returning the URL
  };
  ...
  const handleSubmit = async (e) => {
   ...
   /* if (formData.packageImages.length === 0) {
    ...
    try {
      ...
      const res = await fetch(`${import.meta.env.VITE_SERVER_URL}/api/package/update-package/${params?.id}`, {
        method: "POST",
        headers: {
