...
const Booking = () => {
  ...
  const getPackageData = async () => {
    try {
      ...
      const res = await fetch(
        `${import.meta.env.VITE_SERVER_URL}/api/package/get-package-data/${params?.packageId}`
      );
  ...
  const getToken = async () => {
    try {
      const { data } = await axios.get(`${import.meta.env.VITE_SERVER_URL}/api/package/braintree/token`,
        {
        credentials: "include",
        } 
      );
  ...
  const handleBookPackage = async () => {
    ...
    try {
      setLoading(true);
      const res = await fetch(`${import.meta.env.VITE_SERVER_URL}/api/booking/book-package/${params?.id}`, {
        method: "POST",
        headers: {
