...
const AllPackages = () => {
  ...
  const getPackages = async () => {
    ...
    try {
      ...
      let url =
        filter === "offer" //offer
          ? `${import.meta.env.VITE_SERVER_URL}/api/package/get-packages?searchTerm=${search}&offer=true`
          : filter === "latest" //latest
          ? `${import.meta.env.VITE_SERVER_URL}/api/package/get-packages?searchTerm=${search}&sort=createdAt`
          : filter === "top" //top rated
          ? `${import.meta.env.VITE_SERVER_URL}/api/package/get-packages?searchTerm=${search}&sort=packageRating`
          : `${import.meta.env.VITE_SERVER_URL}/api/package/get-packages?searchTerm=${search}`; //all
  ...
  const onShowMoreSClick = async () => {
    ...
    let url =
      filter === "offer" //offer
        ? `${import.meta.env.VITE_SERVER_URL}/api/package/get-packages?searchTerm=${search}&offer=true&startIndex=${startIndex}`
        : filter === "latest" //latest
        ? `${import.meta.env.VITE_SERVER_URL}/api/package/get-packages?searchTerm=${search}&sort=createdAt&startIndex=${startIndex}`
        : filter === "top" //top rated
        ? `${import.meta.env.VITE_SERVER_URL}/api/package/get-packages?searchTerm=${search}&sort=packageRating&startIndex=${startIndex}`
        : `${import.meta.env.VITE_SERVER_URL}/api/package/get-packages?searchTerm=${search}&startIndex=${startIndex}`; //all
    const res = await fetch(url);
    const data = await res.json();
  ...
  const handleDelete = async (packageId) => {
    try {
      setLoading(true);
      const res = await fetch(`${import.meta.env.VITE_SERVER_URL}/api/package/delete-package/${packageId}`, {
        method: "DELETE",
        credentials:"include",
      });
      const data = await res.json();
