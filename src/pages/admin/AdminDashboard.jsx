...
const AdminDashboard = () => {
  ...
  const handleProfilePhoto = (photo) => {
    try {
      ...
      formData.append("file", photo); // photo file
      formData.append("upload_preset", "ai-travel"); // Replace with your Cloudinary upload preset
      formData.append("cloud_name", import.meta.env.VITE_CLOUDINARY_CLOUD_NAME);//"dhb4g5xox"); // Replace with your Cloudinary cloud name
      ...
      fetch(import.meta.env.VITE_CLOUDINARY, {
        ...
        .then(async (data) => {
          if (data?.secure_url) {
            // Successfully uploaded to Cloudinary, now update user profile with the URL
            const res = await fetch(`${import.meta.env.VITE_SERVER_URL}/api/user/update-profile-photo/${currentUser._id}`, {
              method: "POST",
              headers: {
                "Content-Type": "application/json",
              },
              body: JSON.stringify({ avatar: data.secure_url }), // Cloudinary URL
              credentials: "include",
              });
            const responseData = await res.json();
            if (responseData?.success) {
              alert(responseData?.message);
              setFormData({ ...formData, avatar: data.secure_url }); // Set the new avatar URL
              dispatch(updateUserSuccess(responseData?.user)); // Update user state
              setProfilePhoto(null); // Reset profile photo state
              return;
            } else {
              dispatch(updateUserFailure(responseData?.message)); // Handle error if any
            }
          } else {
