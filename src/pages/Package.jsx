...
const Package = () => {
  ...
  const giveRating = async () => {
    ...
    try {
      setLoading(true);
      const res = await fetch(`${import.meta.env.VITE_SERVER_URL}/api/rating/give-rating`, {
        method: "POST",
        headers: {
  ...
  const getRatings = async () => {
    try {
      const res = await fetch(`${import.meta.env.VITE_SERVER_URL}/api/rating/get-ratings/${params.id}/4`,
          {
          credentials: "include",
          }
      );
  ...
  const checkRatingGiven = async () => {
    try {
      const res = await fetch(
        `${import.meta.env.VITE_SERVER_URL}/api/rating/rating-given/${currentUser?._id}/${params?.id}`,
          {
          credentials: "include",
          }
      );
            ...
            {packageData?.packageImages.map((imageUrl, i) => (
              <SwiperSlide key={i}>
                <div className="relative h-[400px] w-full overflow-hidden">
                  <div 
                    className="absolute inset-0 w-full h-full bg-center bg-no-repeat bg-cover blur-[5px]"
                    style={{ backgroundImage: `url(${imageUrl})` }}  // Corrected syntax!
                  ></div>
                  <img
                    src={imageUrl}
            ...
            <p className="flex gap-1 text-2xl font-semibold my-3">
              {packageData?.packageOffer ? (
                <>
                  ...
                  -<span>INR {packageData?.packageDiscountPrice}</span>
                ...
                <span>INR {packageData?.packagePrice}</span>
              )}
            </p>
            {/* price */}
            {/* destination */}
